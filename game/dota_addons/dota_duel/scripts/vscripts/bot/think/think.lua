...
function BotController:Think()
	...
	if is_in_base then
		...
		if not self.purchased_items then
			...
			Timers:CreateTimer(3.0, end_of_round)
		end
	...
	local significant_dps = nil

	if bot_hero:GetName() == "npc_dota_hero_skeleton_king" then
		...
		significant_dps = bot_hero:GetHealth() / current_ult_cooldown
	else
		significant_dps = bot_hero:GetHealth() / 30
	end
	...
	local very_high_damage = nil

	if bot_hero:GetName() == "npc_dota_hero_skeleton_king" then
		local dps_to_kill = bot_hero:GetMaxHealth() / EFFECTIVE_REINCARNATION_ULT_COOLDOWN
		very_high_damage = dps_to_kill * VERY_HIGH_DAMAGE_THRESHOLD
	else
		very_high_damage = bot_hero:GetMaxHealth() / 5
	end
...
-- Think function for purchasing items
function BotController:ThinkShop()
	local bot_hero = PlayerResource:GetSelectedHeroEntity(self.bot_id)
	local hero_name = bot_hero:GetName()

	if hero_name == "npc_dota_hero_skeleton_king" then
...
-- Think function the when a round starts
function BotController:ThinkRoundStart()
	local bot_hero = PlayerResource:GetSelectedHeroEntity(self.bot_id)
	local hero_name = bot_hero:GetName()
